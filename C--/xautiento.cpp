...
ll Query(string X) {
	...
	for (int i = 0; i < X.size(); ++i) {
		if (tries.nodes[cur].child[X[i] - 'a'] != -1) {
			cur = tries.nodes[cur].child[X[i] - 'a'];
		}
		else return 0;
	}
	
	ret = tries.nodes[cur].cnt;
...
// Dem so luong xau trong tap S nhan X lam tien to